...
function get_area_regex($area_name) {
    ...
    // Explicit overrides for known structural mismatches
...
foreach ($file_map as $div_id => $filename) {
    ...
    // Find the first occurrence of "No." followed by "Catatan" or "CATATAN"
    $real_first_section_offset = 0;
    if (preg_match("/No\.?\s*catatan/i", $text, $match_cat, PREG_OFFSET_CAPTURE)) {
        $real_first_section_offset = $match_cat[0][1];
    }
    ...
    echo "=== Division $div_id | $filename (First section starts at $real_first_section_offset) ===\n";
    
    foreach ($areas as $idx => $area) {
        $pattern = get_area_regex($area['name']);
        $regex = "/(?:\\b\\d+[\\.\\s]+)?" . $pattern . "/i";
        
        // Area 1 can search from 0, others from $real_first_section_offset + 20
        $offset = ($idx === 0) ? 0 : ($real_first_section_offset + 20);
        $found = false;
