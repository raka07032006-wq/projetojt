...
foreach ($file_map as $div_id => $filename) {
    ...
    // Find the first occurrence of "Catatan" or "CATATAN"
    if (preg_match("/catatan/i", $text, $match_cat, PREG_OFFSET_CAPTURE)) {
        $start_search_offset = max(0, $match_cat[0][1] - 150);
    } else {
    ...
    echo "=== Division $div_id | $filename (Search starts at $start_search_offset) ===\n";
