...
foreach ($file_map as $div_id => $filename) {
    ...
    foreach ($areas as $area) {
        ...
        $offset = 0;
        $found = false;
        while (preg_match("/(?:\\b\\d+[\\.\\s]+)?" . $pattern . "/i", $text, $m, PREG_OFFSET_CAPTURE, $offset)) {
            $match_str = $m[0][0];
            $match_offset = $m[0][1];
            
            // Check if "No." and "Catatan" are present within the next 200 characters
            $context = substr($text, $match_offset, 250);
            if (preg_match("/No\b/i", $context) && preg_match("/catatan/i", $context)) {
                echo "  Match Found: '{$area['name']}' -> offset $match_offset\n";
                $matches_found[] = [
                    'id' => $area['id'],
                    'name' => $area['name'],
                    'offset' => $match_offset
                ...
                $found = true;
                break; // Found the section header, stop searching for this area
            }
